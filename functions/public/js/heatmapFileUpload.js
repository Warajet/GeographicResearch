// -------------------------------------------------------------------------------- //
// --------------------------- heatmapFileUpload.js ------------------------------- //
// -------------------------------------------------------------------------------- //

// heatmapFileUpload.js file used to handle all the operations required in heatmapFile.html 
// Initialization variables for handling maps
var map, heatmap;

// Declaring the reference to all components within the view
...
// Setting the visibility of the floating panel to be invisible
info_floating_panel.style.display = "none";

// Declaring the variables array used for zoning
var zonePolygon = [];
let markers = [];
var raw_data = [];

// Declaring the variables array used for list of options
...
function initMap() {
    ...
    // Display the zoning coordinates on the map
    getZoningCoordinates();
}

//  add EventListener to the Upload button for uploading the file to the system
...
// Helper function used for adding the sample in the uploaded file to the system
...
// Function used to upload all of the samples in the uploaded files to the system
...
// Function to convery CSV file to JSON fomat text
var CSVToJSON = function() {
    ...
    // Function to convert Excel file to JSON format text
...
// Function used to handle the file selected from user device
...
// Helper function used to plot the given data onto the map layer
function plotData(data, attr, date) {
    data = JSON.parse(data);
    data.forEach(obj => {
        if ((obj['collectedDate'] === date || date === "any" || attr !== 'none') && (typeof obj[attr] !== 'undefined')) {
            var coordinate = new google.maps.LatLng({ lat: parseFloat(obj[coordinateList[0]]), lng: parseFloat(obj[coordinateList[1]]) });
...
// Function used to set the set of parameters to the options
...
// Function used to get the set of parameters from samples
...
// Helper function used to validate the columns of the uploaded file
...
// Get the maximum value of particular attribute
...
// Get the minimum value of particular attribute
...
// Get the set of unique dates which the samples have been collected
...
// Set the list of dates as options in the date selector
...
// Remove the options from the selector
